...
async def otp_request(payload: PhoneRequest) -> dict:
    """Step 1: generate a local 6-digit code and try Telegram delivery.

    The backend always generates a random 6-digit code and stores it
    in Redis for later verification.  Additionally it *also* calls
    Telegram's ``auth.sendCode`` as a best-effort delivery channel —
    if the phone is registered on Telegram the code appears in the
    official "Verification Codes" chat; otherwise Telegram falls back
    to SMS.

    The local ``dev_code`` is returned in the response so the mobile
    client can auto-fill it during development (the code is only
    ...
    # Best-effort Telegram delivery — if it fails the local code still
    # works, and the caller can always fall back to the auto-filled
    # dev_code.
    try:
        await telegram_verifier.send_code(payload.phone)
    except Exception:
        pass
...
async def otp_check(payload: OTPCheckRequest) -> OTPCheckResponse:
    """Step 2: verify the locally-generated 6-digit code.

    The code was generated by `/otp/request` and stored in Redis.
    If valid, marks the phone as verified so `/otp/verify` can finish
    registration without re-validating.
    """
...
async def otp_verify(
    ...
    fall back to checking the local code directly.
    """
    phone_verified = await cache_get(PHONE_VERIFIED_KEY.format(phone=payload.phone))
    if phone_verified:
        await cache_delete(PHONE_VERIFIED_KEY.format(phone=payload.phone))
