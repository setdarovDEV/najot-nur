...
export function ScoreRing({
  ...
  const color = v >= 80 ? "#1fa971" : v >= 60 ? "#f5a524" : "#e5484d";
  return (
    <div
      className={`score-ring shrink-0 ${className}`}
      style={{
        width: size,
        height: size,
        background: `conic-gradient(${color} ${v * 3.6}deg, rgba(0,0,0,0.08) 0deg)`,
      }}
    >
      <div
...
/** Small centered spinner for inline loading states. */
export function Spinner({ size = 18 }: { size?: number }) {
  return (
    <div className="flex items-center justify-center gap-2 text-muted">
      <Loader2 size={size} className="animate-spin text-wine" />
    </div>
  );
}
